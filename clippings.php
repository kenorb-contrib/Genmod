 ...
 * @package Genmod
 * @subpackage Charts
 * @version $Id: clippings.php,v 1.6 2006/01/22 19:50:23 roland-d Exp $
 */
...
function add_clipping($clipping) {
	global $cart, $gm_lang, $SHOW_SOURCES;
...
//	else $filetext .= "1 PLAC\r\n2 FORM ".$gm_lang["default_form"]."\r\n";
	...
	for($i=0; $i<$ct; $i++)
	{
		...
		if ($clipping['type']=='indi') {
			...
			for ($k=0; $k<$ft; $k++) {
				$filename = check_media_depth(trim($match[$k][1]));
			...
			$ft = preg_match_all("/\d FILE (.*)/", $record, $match, PREG_SET_ORDER);
			for ($k=0; $k<$ft; $k++) {
				$filename = check_media_depth($match[$k][1]);
				 	$media[$mediacount]=$filename;
				 		$mediacount++;
			   	 	$record = preg_replace("@(\d FILE )".addslashes($match[$k][1])."@", "$1".$filename, $record);
			}
...
if($ct==0) {
	...
	for($i=0; $i<$ct; $i++) {
		...
		else if($clipping['type']=='source') print "<img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["source"]["small"]."\" border=\"0\" alt=\"".$gm_lang["source"]."\" />";
		...
		if($clipping['type']=='indi')
		{
		  if (displayDetailsbyID($clipping['id']) or showLivingNameByID($clipping['id'])) $id_ok = true;
		  else $id_ok = false;
		  if ($id_ok) $dName = get_sortable_name($clipping['id']); else $dName = $gm_lang["person_private"];
		...
		else
		{
		  if($clipping['type']=='fam')
		  {
		    ...
		    if ($ct01 > 0) {
			if (displayDetailsbyID($match[1]) or showLivingNameByID($match[1])) $husb_ok = true;
			else $husb_ok = false;
		    }
		    ...
		    if ($ct02 > 0) {
		      if (displayDetailsByID($match[1]) or showLivingNameByID($match[1])) $wife_ok = true;
		      else $wife_ok = false;
		    }
		    if (($husb_ok) and ($wife_ok)) $dName = get_family_descriptor($clipping['id']); else $dName = $gm_lang["family_private"];
		    $names = preg_split("/,/", $dName);
			$dName = check_NN($names);
