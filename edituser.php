...
if (!isset($user->default_tab)) $user->default_tab = 9;
...
if ($action=="edituser2") {
	...
	if ($username != $oldusername && !$u->is_empty) {
		...
		if ($pass) {
			...
			if ($user->sync_gedcom == "Y" && $sync_data_changed && !empty($user->email)) {
				$oldged = $GEDCOM;
...
<table class="list_table <?php print $TEXT_DIRECTION; ?>">
	...
	<tr><td class="shade2 wrap"><div class="helpicon"><?php print_help_link("edituser_gedcomid_help", "qm"); print "</div><div class=\"description\">"; print $gm_lang["gedcomid"];?></div></td><td class="shade1">
		<?php
			if (!empty($user->gedcomid[$GEDCOM])) {
				print "<ul>";
				print_list_person($user->gedcomid[$GEDCOM], array(GetPersonName($user->gedcomid[$GEDCOM]), $GEDCOM));
				print "</ul>";
			}
			else print "&nbsp;";
		?>
    ...
    ?></td></tr>
    <tr><td class="shade2 wrap"><div class="helpicon"><?php print_help_link("edituser_email_help", "qm"); print "</div><div class=\"description\">"; print $gm_lang["emailadress"];?></div></td><td class="shade1" valign="top"><input type="text" name="user_email" tabindex="<?php $tab++; print $tab; ?>" value="<?php print $user->email; ?>" size="50" onchange="sndReq('errem', 'checkemail', 'email', this.value);" />&nbsp;&nbsp;<span id="errem"></span></td></tr>
    ...
    <tr>
		...
		<td class="shade1"><select name="new_default_tab" tabindex="<?php $tab++; print $tab; ?>">
				<option value="9" <?php if ($user->default_tab==9) print "selected=\"selected\""; ?>><?php print $gm_lang["site_default"]; ?></option>
				...
				<option value="6" <?php if ($user->default_tab==6) print "selected=\"selected\""; ?>><?php print $gm_lang["all"];?></option>
			</select>
		</td>
	</tr>
