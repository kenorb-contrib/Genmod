 ...
 * @package Genmod
 * @subpackage Blocks
 * @version $Id: html_block.php,v 1.3 2006/02/19 18:40:23 roland-d Exp $
 */
...
$GM_BLOCKS["print_html_block"]["config"]      = array("html"=>$gm_lang["html_block_sample_part1"]." <img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["admin"]["small"]."\" alt=\"".$gm_lang["config_block"]."\" /> ".$gm_lang["html_block_sample_part2"]);

function print_html_block($block=true, $config="", $side, $index) {
	global $gm_lang, $GM_IMAGE_DIR, $TEXT_DIRECTION, $GM_IMAGES, $HTML_BLOCK_COUNT, $GM_BLOCKS, $command, $GEDCOM, $gm_username;
	...
	if ($GM_BLOCKS["print_html_block"]["canconfig"]) {
		...
		if ((($command=="gedcom")&&(userGedcomAdmin($username))) || (($command=="user")&&(!empty($username)))) {
			...
			print "<br /><a href=\"javascript: configure block\" onclick=\"window.open('index_edit.php?name=$name&amp;command=$command&amp;action=configure&amp;side=$side&amp;index=$index', '', 'top=50,left=50,width=750,height=550,scrollbars=1,resizable=1'); return false;\">";
