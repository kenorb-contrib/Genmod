 ...
 * @package Genmod
 * @subpackage Blocks
 * @version $Id: review_changes.php,v 1.9 2006/02/27 22:07:26 roland-d Exp $
...
function review_changes_block($block = true, $config="", $side, $index) {
	...
	global $gm_changes, $LAST_CHANGE_EMAIL, $ALLOW_EDIT_GEDCOM, $SERVER_URL, $TEXT_DIRECTION, $SHOW_SOURCES, $TIME_FORMAT, $GM_BLOCKS, $TBLPREFIX, $TOTAL_QUERIES, $gm_username;
	...
	if ($display_block && userCanEdit($gm_username)) {
		...
		if (userCanAccept($gm_username)) print "<a href=\"#\" onclick=\"window.open('edit_changes.php','','width=600,height=600,resizable=1,scrollbars=1'); return false;\">".$gm_lang["accept_changes"]."</a><br />\n";
		...
		if ($config["sendmail"]=="yes" && $LAST_CHANGE_EMAIL != 0) {
			...
			print $gm_lang["last_email_sent"].get_changed_date("$day $mon $year")." - ".date($TIME_FORMAT, $LAST_CHANGE_EMAIL)."<br />\n";
			...
			print $gm_lang["next_email_sent"].get_changed_date("$day $mon $year")." - ".date($TIME_FORMAT, $LAST_CHANGE_EMAIL+(60*60*24*$config["days"]))."<br /><br />\n";
		}
		$gm_changes = GetChangeData(false, "", true, "gedlines");
		foreach($gm_changes as $gedcom=>$changes) {
			if ($gedcom == $GEDCOM) {
				foreach($changes as $gid=>$change) {
					$ct = preg_match("/0 @(.*)@(.*)/", $change, $match);
					if ($ct>0) $type = trim($match[2]);
					else $type = "INDI";
					if ($type=="INDI") print "<b>".PrintReady(get_person_name($gid))."</b> &lrm;(".$gid.")&lrm;\n";
					else if ($type=="FAM") print "<b>".PrintReady(get_family_descriptor($gid))."</b> &lrm;(".$gid.")&lrm;\n";
					else if ($type=="SOUR") {
						if ($SHOW_SOURCES>=getUserAccessLevel($gm_username)) print "<b>".PrintReady(get_source_descriptor($gid))."</b> &lrm;(".$gid.")&lrm;\n";
					}
					else if ($type=="REPO") {
						if ($SHOW_SOURCES>=getUserAccessLevel($gm_username)) print "<b>".PrintReady(get_repo_descriptor($gid))."</b> &lrm;(".$gid.")&lrm;\n";
					}
					else print "<b>".$factarray[$type]."</b> &lrm;(".$gid.")&lrm;\n";
					if ($block) print "<br />";
					if ($type=="INDI") print " <a href=\"individual.php?pid=".$gid."&amp;ged=".$gedcom."&amp;show_changes=yes\">".$gm_lang["view_change_diff"]."</a>\n<br />";
					if ($type=="FAM") print " <a href=\"family.php?famid=".$gid."&amp;ged=".$gedcom."&amp;show_changes=yes\">".$gm_lang["view_change_diff"]."</a>\n<br />";
					if (($type=="SOUR") && ($SHOW_SOURCES>=getUserAccessLevel($gm_username))) print " <a href=\"source.php?sid=".$gid."&amp;ged=".$gedcom."&amp;show_changes=yes\">".$gm_lang["view_change_diff"]."</a>\n<br />";
					if (($type=="REPO") && ($SHOW_SOURCES>=getUserAccessLevel($gm_username))) print " <a href=\"repo.php?rid=".$gid."&amp;ged=".$gedcom."&amp;show_changes=yes\">".$gm_lang["view_change_diff"]."</a>\n<br />";
