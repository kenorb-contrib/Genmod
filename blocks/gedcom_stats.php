 ...
 *
 * @version $Id: gedcom_stats.php,v 1.8 2006/04/08 10:52:00 sjouke Exp $
 * @package Genmod
 * @subpackage Blocks
 */
...
function print_gedcom_stats($block = true, $config="", $side, $index) {
		...
		global $top10_block_present, $TBLPREFIX, $monthtonum, $gm_username;		// Set in index.php
		...
		if ($GM_BLOCKS["print_gedcom_stats"]["canconfig"]) {
			...
			if ((($command=="gedcom")&&(userGedcomAdmin($username))) || (($command=="user")&&(!empty($username)))) {
				...
				print "<a href=\"javascript: configure block\" onclick=\"window.open('index_edit.php?name=$name&amp;command=$command&amp;action=configure&amp;side=$side&amp;index=$index', '', 'top=50,left=50,width=600,height=350,scrollbars=1,resizable=1'); return false;\">";
		...
		print $stats["gs_title"];
		
		print "<br />\n";
		print "<table><tr><td valign=\"top\" class=\"width20 wrap\"><table cellspacing=\"1\" cellpadding=\"0\">";
		if ($config["stat_indi"]=="yes") print "<tr><td>".$gm_lang["stat_individuals"]." </td><td class=\"rtl\"><b>&nbsp;<a href=\"indilist.php?surname_sublist=no\">".get_list_size("indilist")."</a></b></td></tr>";
		...
		if ($config["stat_users"]=="yes") print "<tr><td>".$gm_lang["stat_users"]." </td><td class=\"rtl\"><b>&nbsp;".count(getUsers())."</b></td></tr>";
		...
		if ($config["stat_first_birth"]=="yes") {
			...
			if (displayDetailsById($stats["gs_earliest_birth_gid"])) print_list_person($stats["gs_earliest_birth_gid"], array(get_person_name($stats["gs_earliest_birth_gid"]), $GEDCOM), false, "", false);
			print "</td></tr>\n";
		}
		if ($config["stat_last_birth"]=="yes") {
			...
			if (displayDetailsById($stats["gs_latest_birth_gid"])) print_list_person($stats["gs_latest_birth_gid"], array(get_person_name($stats["gs_latest_birth_gid"]), $GEDCOM), false, "", false);
			print "</td></tr>\n";
		}
		if ($config["stat_long_life"]=="yes") {
			...
			if (displayDetailsById($stats["gs_longest_live_gid"])) print_list_person($stats["gs_longest_live_gid"], array(get_person_name($stats["gs_longest_live_gid"]), $GEDCOM), false, "", false);
			print "</td></tr>\n";
		}
		...
		if ($config["stat_most_chil"]=="yes") {
			...
			if (!empty($stats["gs_most_children_nr"])) {
				...
				if (displayDetailsById($stats["gs_most_children_gid"], "FAM")) print_list_family($stats["gs_most_children_gid"], array(get_family_descriptor($stats["gs_most_children_gid"]), $GEDCOM), false, "", false);
				print "</td></tr>\n";
			}
		}
