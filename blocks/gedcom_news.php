 ...
 *
 * @version $Id: gedcom_news.php,v 1.4 2006/02/27 22:07:26 roland-d Exp $
 * @package Genmod
 * @subpackage Blocks
 */
...
function print_gedcom_news($block = true, $config="", $side, $index) {
		global $gm_lang, $GM_IMAGE_DIR, $GM_IMAGES, $TEXT_DIRECTION, $GEDCOM, $command, $TIME_FORMAT, $VERSION, $gm_username;

		$uname = $gm_username;
		$usernews = getUserNews($GEDCOM);
		print "<div id=\"gedcom_news\" class=\"block\">\n";
		print "<div class=\"blockhc\">";
		if (userGedcomAdmin($gm_username)) print_help_link("index_gedcom_news_ahelp", "qm_ah");
		...
		if ($block) print "</div>\n";
		if (userGedcomAdmin($uname)) print "<a href=\"#\" onclick=\"addnews('".preg_replace("/'/", "\'", $GEDCOM)."'); return false;\">".$gm_lang["add_news"]."</a>\n";
		print "</div>\n";
		print "</div>";
