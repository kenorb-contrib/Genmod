 ...
 *
 * @version $Id: user_welcome.php,v 1.4 2006/04/14 22:40:34 roland-d Exp $
 * @package Genmod
 * @subpackage Blocks
 */
...
function print_welcome_block($block=true, $config="", $side, $index) {
		global $gm_lang, $day, $month, $year, $GM_IMAGE_DIR, $GM_IMAGES, $user, $GEDCOM, $TIME_FORMAT,$command, $gm_username;

		print "<div id=\"user_welcome\" class=\"block\">\n";
		print "<div class=\"blockhc\">";
		print $gm_lang["welcome"]." ".$user["firstname"]." ".$user["lastname"];
		print "</div>";
		...
		if (!empty($user["gedcomid"][$GEDCOM])) {
			print "<div><a href=\"pedigree.php?rootid=".$user["gedcomid"][$GEDCOM]."\"><img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["pedigree"]["small"]."\" border=\"0\" alt=\"".$gm_lang["my_pedigree"]."\" title=\"".$gm_lang["my_pedigree"]."\" /><br />".$gm_lang["my_pedigree"]."</a></div>";
			print "<div><a href=\"individual.php?pid=".$user["gedcomid"][$GEDCOM]."\"><img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["indis"]["small"]."\" border=\"0\" alt=\"".$gm_lang["my_indi"]."\" title=\"".$gm_lang["my_indi"]."\" /><br />".$gm_lang["my_indi"]."</a></div>\n";
		}
		print "<div>";
		...
		print "\n<div>".get_changed_date("$day $month $year")." - ".date($TIME_FORMAT, time()-$_SESSION["timediff"])."</div>\n";
