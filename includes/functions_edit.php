 ...
 * @version $Id: functions_edit.php,v 1.139 2009/03/16 19:51:12 sjouke Exp $
 */
		...
		if ($value) print GetRepoDescriptor($value)." (".$value.")";
			...
			if (isset($_SESSION["last_used"]["REPO"])) {
				...
				if ($gedid == $GEDCOMID) {
					...
					print "<a href=\"javascript\" onclick=\"document.getElementById('".$element_id."').value='".$id."'; sndReq('".$element_id."_repo', 'getrepodescriptor', 'rid', '".$id."', '', ''); return false;\">".$gm_lang["click_for"]." ".GetRepoDescriptor($id)."</a>";
				}
			}
		}
		print "</span>";
		...
		<?php
			...
			if (isset($_SESSION["last_used"]["NOTE"])) {
				...
				if ($gedid == $GEDCOMID) {
					...
					print "<a href=\"javascript\" onclick=\"document.getElementById('".$element_id."').value='".$id."'; sndReq('".$element_id."_gnote', 'getnotedescriptor', 'oid', '".$id."', '', ''); return false;\">".$gm_lang["click_for"]." <span id=\"".$element_id."_gnote2\"></span></a>";
					?>
					<script>
		...
		if ($value) print GetSourceDescriptor($value)." (".$value.")";
			...
			if (isset($_SESSION["last_used"]["SOUR"])) {
				...
				if ($gedid == $GEDCOMID) {
					...
					print "<a href=\"javascript\" onclick=\"document.getElementById('".$element_id."').value='".$id."'; sndReq('".$element_id."_src', 'getsourcedescriptor', 'sid', '".$id."', '', ''); return false;\">".$gm_lang["click_for"]." ".GetSourceDescriptor($id)."</a>";
				}
			}
		}
		print "</span>";
...
function HandleUpdates($newged) {
	global $glevels, $islink, $tag, $uploaded_files, $text, $NOTE, $change_id, $change_type, $success, $GEDCOM, $can_auto_accept;
//	print_r($_POST);
...
//		if (!empty($text[$j]) || $pass == true  || $text[$j] == "0") {
		if ($pass == true) {			
			...
			else if(!$skip) $newline = $glevels[$j]." ".$tag[$j];
