	...
	private function GetBlocks() {
		...
		if ($res->NumRows() > 0) {
			while($row = $res->FetchAssoc()){
				...
				if ($row["b_location"]=="main") $this->main[$row["b_order"]] = array($row["b_name"], unserialize($row["b_config"]));
				if ($row["b_location"]=="right") $this->right[$row["b_order"]] = array($row["b_name"], unserialize($row["b_config"]));
			}
			$res->FreeResult();
		}
		else {
			if ($this->type == "user") {
				...
				if ($res2->NumRows() > 0) {
					while($row = $res2->FetchAssoc()){
						if (!isset($row["b_config"])) $row["b_config"]="";
						if ($row["b_location"]=="main") $this->blocks["main"][$row["b_order"]] = array($row["b_name"], unserialize($row["b_config"]));
						if ($row["b_location"]=="right") $this->blocks["right"][$row["b_order"]] = array($row["b_name"], unserialize($row["b_config"]));
					}
					$res2->FreeResult();
				}
