 ...
 *
 * @package Genmod
 * @version $Id: functions_name.php,v 1.69 2009/02/28 13:04:33 sjouke Exp $
 */
...
function GetAddPersonName($pid, $record="") {
	global $NAME_FROM_GEDCOM;

	//-- get the name from the indexes
	if (empty($record)) $record = FindPersonRecord($pid);
	$name_record = GetSubRecord(1, "1 NAME", $record);
	$name = GetAddPersonNameInRecord($name_record);
	return $name;
}

function GetAddPersonNameInRecord($name_record, $keep_slash=false) {
	global $NAME_REVERSE;
	global $NAME_FROM_GEDCOM;
...
function GetPinYin($name, $import = false) {
	global $pinyin, $GM_BASEDIRECTORY;
...
function GetGBcode($name) {
	global $GBcode, $GM_BASEDIRECTORY;
	$gbstr = "";
	$pos1 = 0;
	$pos2 = 2;
	while ($pos2 < strlen($name)) {
		...
		if (HasChinese($char, true)) {
			if (!isset($GBcode)) {
				$fname = $GM_BASEDIRECTORY."includes/gbcode.php";
				require($fname);
			}
