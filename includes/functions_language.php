 ...
 *
 * @package Genmod
 * @version $Id: functions_language.php,v 1.3 2009/03/18 18:47:59 sjouke Exp $
 */

/**
 * security check to prevent hackers from directly accessing this file
 */
if (strstr($_SERVER["SCRIPT_NAME"],"functions_language.php")) {
...
//			print $data[0]." - ".$data[1]."<br />";
			...
			if (!isset($data[1])) WriteToLog($line, "E", "S");
				...
				$sql = "INSERT INTO ".$TBLPREFIX."facts (lg_string, lg_english, lg_last_update_date, lg_last_update_by) VALUES ('".mysql_real_escape_string($data[0])."', '".mysql_real_escape_string($data[1])."', '".time()."', '".$gm_username."')";
				if (!$result = NewQuery($sql)) {
...
function WriteString($string, $value, $language2, $file_type="") {
	...
	if ($file_type == "facts") {
		...
		if ($language2 == "english") {
			...
			$sql .= " (lg_string, lg_".$language2.") VALUES ('".$value."', '".$string."') ON DUPLICATE KEY UPDATE lg_".$language2."= '".$string."'";
		}
		else {
	...
	if (!isset($implangs)) {
		...
		if ($res->NumRows() > 0) {
			...
			while($lang = $res->FetchRow()) {
				...
				if (file_exists($language_settings[$lang[0]]["gm_language"]) && file_exists($language_settings[$lang[0]]["helptextfile"]) && file_exists($language_settings[$lang[0]]["factsfile"])) {
