 ...
 * @package Genmod
 * @subpackage Lists
 * @version $Id: actionlist.php,v 1.6 2009/03/25 16:53:46 sjouke Exp $
 */
...
if (count($actionlist) == 0) {
	...
	if ($sort == "person") {
		...
		foreach($actionlist as $key => $action) {
			if (empty($action->pid)) {
				print "<tr><td class=\"shade1\">&nbsp;</td><td class=\"shade1\"><a href=\"repo.php?rid=".$action->repo."\">".GetRepoDescriptor($action->repo)."</a></td><td class=\"shade1\">".$gm_lang["action_".$action->status]."</td><td class=\"shade1 wrap\">".nl2br(stripslashes($action->text))."</td></tr>";
			}
		}
		...
		foreach($indis as $ipid=>$indi) {
			foreach($actionlist as $key => $action) {
				if ($action->pid == $ipid) {
					...
					print "</td><td class=\"shade1\">".$gm_lang["action_".$action->status]."</td><td class=\"shade1 wrap\">".nl2br(stripslashes($action->text))."</td></tr>";
				}
			}
		}
	}
