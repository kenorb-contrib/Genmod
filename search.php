 ...
 * @package Genmod
 * @subpackage Display
 * @version $Id: search.php,v 1.76 2009/03/29 15:54:54 sjouke Exp $
 */
...
if ($action=="soundex") {
	if (((!empty($lastname))||(!empty($firstname))||(!empty($place)))&&(count($sgeds)>0)) {
		...
		// Build the query
		$sql = "SELECT i_key, i_id, i_gedcom, i_file, i_isdead, n_name, n_surname, n_type, n_letter FROM ";
			...
			if ($save === true) {
				if ($nameprt == "all") {
					foreach($value["names"] as $indexval => $namearray) {
						$printname[] = array(SortableNameFromName($namearray[0]), $ikey, get_gedcom_from_id($value["gedfile"]),"");
					}
				}
				else {
					foreach($value["names"] as $indexval => $namearray) {
						...
						if ($print) $printname[] = array(SortableNameFromName($namearray[0]), $ikey, get_gedcom_from_id($value["gedfile"]),"");
					}
				}
			}
		}
		...
		// We may add the assos at this point.
			...
			foreach($printname as $key => $pname) {
				$apid = $pname[1]."[".get_id_from_gedcom($pname[2])."]";
					...
					foreach($assolist[$apid] as $indexval => $asso) {
						if ($asso["type"] == "indi") {
							...
							foreach($asso["name"] as $nkey => $assoname) {
								...
								$printname[] = array(SortableNameFromName($assoname[0]), $key, get_gedcom_from_id($asso["gedfile"]), $apid);
							}
						}
							...
							foreach($asso["name"] as $nkey => $assoname) {
								...
								if (hasRTLText($assosplit[0]) == hasRTLText($assosplit[1]) && HasChinese($assosplit[0], true) == HasChinese($assosplit[1], true)) {
									$apid2 = SplitKey($indexval, "id");
									$printfamname[]=array(CheckNN($assoname), $apid2, get_gedcom_from_id($asso["gedfile"]), $apid);
								}
							}
						}
					}
		...
		if (count($printname)==1) {
			...
			$GEDCOMID = $GEDCOMS[$GEDCOM]["id"];
			$Privacy->ReadPrivacy($GEDCOMID);
			if (showLivingNameByID($printname[0][1])) {
		...
		if ($sorton == "last" || $sorton == "") uasort($printname, "ItemSort");
						...
						if (isset($assolist[$apid])) {
							...
							foreach($assolist[$apid] as $indexval => $asso) {
								if ($asso["type"] == "indi") {
									...
									foreach($asso["name"] as $nkey => $assoname) {
										$key = SplitKey($indexval, "id");
										$printindiname[] = array(SortableNameFromName($assoname[0]), $key, get_gedcom_from_id($asso["gedfile"]), $apid);
									}
								}
								else if ($asso["type"] == "fam") {
									$fam_printed[$indexval] = "1";
