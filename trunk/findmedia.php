...
if ($embed) {
	$thistitle = $gm_lang["manage_media"];
	print_header($thistitle);
}else{
	$thistitle = $gm_lang["find_media"];
...
<form action="findmedia.php" method="GET">
	...
	<div class="center">
		<table class="list-table">
			...
			<tr>
				<td />
		    	<td class="list-label <?php print $TEXT_DIRECTION; ?>" ><input type="checkbox" name="showthumb" value="true" <?php if( $showthumb) {print "checked=\"checked\"";} ?>" onclick="javascript: this.form.submit()"><?php print $gm_lang["show_thumbnail"];?>
...
if ($action == "newdir") {
	...
	if ($clean) {
		...
		if (file_exists($directory."index.php")) {
			...
			if (!$fp) {
				print "<div class=\"error\">".$gm_lang["security_no_create"].$directory.$newdir."</div>";
			} else {
				fputs($fp,$inddata);
				fclose($fp);
			}
		} else {
			print "<div class=\"error\">".$gm_lang["security_not_exist"].$directory."</div>";
		}
		if (file_exists($thumbdir."index.php")) {
			...
			if (!$fp) {
				print "<div class=\"error\">".$gm_lang["security_no_create"].$thumbdir.$newdir."</div>";
			} else {
				fputs($fp,$inddata);
				fclose($fp);
			}
		} else {
			print "<div class=\"error\">".$gm_lang["security_not_exist"].$thumbdir."</div>";
		}
	} else {
...
function print_move_to_menu($dirlist,$directory, $filename) {
	global $level, $MEDIA_DIRECTORY_LEVELS, $embed, $gm_lang, $thumbget;
