 ...
 * @package Genmod
 * @subpackage Edit
 * @version $Id: edit_merge.php,v 1.4 2006/01/09 14:19:29 sjouke Exp $
 */
/**
...
/**
 * Inclusion of the language fact files
*/
...
if ($action!="choose") {
	if ($gid1==$gid2 && $GEDCOM==$ged2) {
		$action="choose";
		print "<span class=\"error\">".$gm_lang["same_ids"]."</span>\n";
	}
	else {
		if (!isset($gm_changes[$gid1."_".$GEDCOM])) $gedrec1 = find_gedcom_record($gid1);
		if ($GEDCOM!=$ged2) $GEDCOM=$ged2;
		if (!isset($gm_changes[$gid2."_".$ged2])) $gedrec2 = find_gedcom_record($gid2);
		$GEDCOM=$ged;

		if (empty($gedrec1) || empty($gedrec2)) {
			print "<span class=\"error\">".$gm_lang["unable_to_find_record"]."</span>\n";
			$action="choose";
		}
		else {
			$type1 = "";
			...
			if (!empty($type1) && ($type1!=$type2)) {
				print "<span class=\"error\">".$gm_lang["merge_same"]."</span>\n";
				$action="choose";
			}
			else {
				$facts1 = array();
...
//				$ct = preg_match_all("/\n1 (_?[A-Z]{3,5})(.*)/", $gedrec2, $match, PREG_SET_ORDER);
				...
				if ($action=="select") {
					print "<h2>".$gm_lang["merge_step2"]."</h2>\n";
					print "<form method=\"post\" action=\"edit_merge.php\">\n";
					print $gm_lang["merge_facts_same"]."<br />\n";
					...
					print "</table><br /><br />\n";
					print $gm_lang["unmatching_facts"]."<br />\n";
					...
					print "</td></tr>\n";
					print "</table>\n";
					print "<input type=\"submit\" value=\"".$gm_lang["merge_records"]."\">\n";
					print "</form>\n";
				}
				else if ($action=="merge") {
					$manual_save = true;
					print "<h2>".$gm_lang["merge_step3"]."</h2>\n";
					...
					for($i=0; ($i<count($facts1) || $i<count($facts2)); $i++) {
						if (isset($facts1[$i])) {
							if (in_array($i, $keep1)) {
								...
								print $gm_lang["adding"]." ".$facts1[$i]["fact"]." ".$gm_lang["from"]." $gid1<br />\n";
							}
						}
						if (isset($facts2[$i])) {
							if (in_array($i, $keep2)) {
								$newgedrec .= $facts2[$i]["subrec"]."\r\n";
								print $gm_lang["adding"]." ".$facts2[$i]["fact"]." ".$gm_lang["from"]." $gid2<br />\n";
							}
						}
					}
					...
					foreach($records as $indexval => $record) {
						...
						if (strstr($record, "@$gid2@")!==false) {
							print $gm_lang["updating_linked"]." $gid<br />\n";
							$newrec = "0".$record;
					...
					print "<br /><a href=\"edit_merge.php?action=choose\">".$gm_lang["merge_more"]."</a><br />\n";
					print "<br /><br /><br />\n";
				}
			}
		}
...
if ($action=="choose") {
	...
	print "</select>\n";
	print "<a href=\"javascript:iopen_find(document.merge.gid2, document.merge.ged2);\"> ".$gm_lang["find_individual"]."</a> |";
	print "<a href=\"javascript:fopen_find(document.merge.gid2, document.merge.ged2);\"> ".$gm_lang["find_familyid"]."</a> |";
	print "<a href=\"javascript:sopen_find(document.merge.gid2, document.merge.ged2);\"> ".$gm_lang["find_sourceid"]."</a>";
	print_help_link("rootid_help", "qm");
