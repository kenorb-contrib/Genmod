 ...
 * @package Genmod
 * @subpackage Charts
 * @version $Id: hourglass.php,v 1.4 2006/03/08 20:46:01 roland-d Exp $
 */
	...
	if ($count==0) {
		...
		if (displayDetailsByID($pid) || showLivingNameByID($pid)) {
			...
			if ($famids||($num>1)) {
				...
				if ($view!="preview") {
					print "<a href=\"javascript: ".$gm_lang["show"]."\" onclick=\"togglechildrenbox(); return false;\" onmouseover=\"swap_image('larrow',3);\" onmouseout=\"swap_image('larrow',3);\">";
...
print_header(PrintReady($name)." ".$gm_lang["hourglass_chart"]);
// NOTE: Start table header
print "\n\t<table width=\"100%\" class=\"list_table $TEXT_DIRECTION\"><tr><td valign=\"top\">\n\t\t";
if ($view!="preview") print "\n\t<h2>".$gm_lang["hourglass_chart"].":<br />".PrintReady($name)."</h2>";
else print "\n\t<h2 style=\"text-align: center\">".$gm_lang["hourglass_chart"].":<br />".PrintReady($name)."</h2>";
?>
