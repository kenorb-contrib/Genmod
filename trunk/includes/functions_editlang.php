 ...
 * @package Genmod
 * @subpackage Admin
 * @version $Id: functions_editlang.php,v 1.3 2006/02/19 11:32:04 roland-d Exp $
 */
...
{
  global $file_type, $language2, $GM_BASE_DIRECTORY, $lang_shortcut, $gm_username;

  $Filename = $GM_BASE_DIRECTORY . $dFileName;
  LockFile($Filename);
  ...
  if (!$dFound)
  {
    ...
    switch ($file_type)
    {
      ...
      case "configure_help"	: fwrite($fp, "/*=============================================================" . "\r\n");
      			  	  ...
      			  	  fwrite($fp, "   Author:	John Finlay" . "\r\n");
      			  	  fwrite($fp, "   Translation:	" . $dUserRealName . "\r\n");
      			  	  fwrite($fp, "   Comments:	" . $language2 . " Language Configure Help file for Genmod" . "\r\n");
      			  	  fwrite($fp, "   Change Log:	" . date("Y") . "-" . date("m") . "-" . date("d") . " - File created by " . $dUserRealName . "\r\n");
      			  	  fwrite($fp, "   		For other changes read:" . "\r\n");
      			  	  fwrite($fp, "   		languages/LANG_CHANGELOG.txt" . "\r\n");
      			  	  fwrite($fp, "=============================================================*/" . "\r\n");
      ...
      case "help_text"		: fwrite($fp, "/*=============================================================" . "\r\n");
      			  	  ...
      			  	  fwrite($fp, "   Author:	John Finlay" . "\r\n");
      			  	  fwrite($fp, "   Translation:	" . $dUserRealName . "\r\n");
      			  	  fwrite($fp, "   Comments:	" . $language2 . " Language Help-file for Genmod" . "\r\n");
      			  	  fwrite($fp, "   Change Log:	" . date("Y") . "-" . date("m") . "-" . date("d") . " - File created by " . $dUserRealName . "\r\n");
      			  	  fwrite($fp, "   		For other changes read:" . "\r\n");
      			  	  fwrite($fp, "   		languages/LANG_CHANGELOG.txt" . "\r\n");
      			  	  fwrite($fp, "=============================================================*/" . "\r\n");
...
function find_in_file($MsgNr, $dlang_file)
{
  ...
  $openfilename = $GM_BASE_DIRECTORY . $dlang_file;
...
function write_array_into_file($dFileName01, $writeArray, $add_new_message_at_line, $new_message_string)
{
  ...
  $Filename = $GM_BASE_DIRECTORY . $dFileName01;
  # $Filename = $GM_BASE_DIRECTORY . "languages/test.php";
  LockFile($Filename);
...
//-----------------------------------------------------------------
// Function below might be obsolete, need to check
// Reason: languages in DB now
...
function check_bom(){
	...
	foreach ($language_settings as $key => $language) {
		...
		if ($language["gm_lang_use"] == true) {
			// Check language file
			if (file_exists($language["gm_language"])) $str = file_get_contents($language["gm_language"]);
			else {
				print "<span class=\"warning\">";
				print str_replace("#lang_filename#", substr($language["gm_language"], 10), $gm_lang["no_open"]) . "<br /><br />";
				print "</span>";
			}
			if (ord($str{0}) == 239 && ord($str{1}) == 187 && ord($str{2}) == 191) {
				$check = true;
				print "<span class=\"warning\">".$gm_lang["bom_found"].substr($language["gm_language"], 10).".</span>";
				print "<br />";
				...
				if (@fwrite($handle,html_entity_decode($writetext)) === FALSE) {
	       			print "<span class=\"warning\">";
	          		print str_replace("#lang_filename#", substr($language["gm_language"], 10), $gm_lang["lang_file_write_error"]) . "<br /><br />";
	          		print "</span>";
	   			}
			}
			...
			if (file_exists($language["confighelpfile"])) $str = file_get_contents($language["confighelpfile"]);
				...
				print "<span class=\"warning\">";
				print str_replace("#lang_filename#", substr($language["confighelpfile"], 10), $gm_lang["no_open"]) . "<br /><br />";
				print "</span>";
			}
			if (ord($str{0}) == 239 && ord($str{1}) == 187 && ord($str{2}) == 191) {
				$check = true;
				print "<span class=\"warning\">".$gm_lang["bom_found"].substr($language["confighelpfile"], 10).".</span>";
				print "<br />";
				$writetext = htmlentities(substr($str,3, strlen($str)));
				...
				if (@fwrite($handle,html_entity_decode($writetext)) === FALSE) {
	       			print "<span class=\"warning\">";
	          		print str_replace("#lang_filename#", substr($language["confighelpfile"], 10), $gm_lang["lang_file_write_error"]) . "<br /><br />";
	          		print "</span>";
	   			}
			}
			...
			if (file_exists($language["helptextfile"])) $str = file_get_contents($language["helptextfile"]);
				...
				print "<span class=\"warning\">";
				print str_replace("#lang_filename#", substr($language["helptextfile"], 10), $gm_lang["no_open"]) . "<br /><br />";
				print "</span>";
			}
			if (ord($str{0}) == 239 && ord($str{1}) == 187 && ord($str{2}) == 191) {
				$check = true;
				print "<span class=\"warning\">".$gm_lang["bom_found"].substr($language["helptextfile"], 10).".</span>";
				print "<br />";
				$writetext = htmlentities(substr($str,3, strlen($str)));
				...
				if (@fwrite($handle,html_entity_decode($writetext)) === FALSE) {
	       			print "<span class=\"warning\">";
	          		print str_replace("#lang_filename#", substr($language["helptextfile"], 10), $gm_lang["lang_file_write_error"]) . "<br /><br />";
	          		print "</span>";
	   			}
			}
			...
			if (file_exists($language["factsfile"])) $str = file_get_contents($language["factsfile"]);
				...
				print "<span class=\"warning\">";
				print str_replace("#lang_filename#", substr($language["factsfile"], 10), $gm_lang["no_open"]) . "<br /><br />";
				print "</span>";
			}
			if (ord($str{0}) == 239 && ord($str{1}) == 187 && ord($str{2}) == 191) {
				$check = true;
				print "<span class=\"warning\">".$gm_lang["bom_found"].substr($language["factsfile"], 10).".</span>";
				print "<br />";
				$writetext = htmlentities(substr($str,3, strlen($str)));
				...
				if (@fwrite($handle,html_entity_decode($writetext)) === FALSE) {
	       			print "<span class=\"warning\">";
	          		print str_replace("#lang_filename#", substr($language["factsfile"], 10), $gm_lang["lang_file_write_error"]) . "<br /><br />";
	          		print "</span>";
	   			}
			}
		}
	}
	if ($check == false) print $gm_lang["bom_not_found"];
}
