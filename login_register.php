 ...
 * @package Genmod
 * @subpackage Admin
 * @version $Id: login_register.php,v 1.30 2009/01/07 18:03:07 sjouke Exp $
 */
	...
	case "verify_hash" :
		...
		if (!$user->is_empty) {
			...
			if (($pw_ok) and ($hc_ok)) {
				...
				$mail_body .= "<a href=\"http://".$_SERVER['SERVER_NAME'] . $path."/useradmin.php?action=edituser&username=" . urlencode($newuser->username)."\">"."http://".$_SERVER['SERVER_NAME'] . $path."/useradmin.php?action=edituser&username=" . urlencode($newuser->username) . "</a>\r\n";
