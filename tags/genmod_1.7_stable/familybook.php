 ...
 * @package Genmod
 * @subpackage Charts
 * @version $Id: familybook.php,v 1.2 2006/01/07 23:19:17 roland-d Exp $
 */
...
function print_descendency($pid, $count) {
	global $show_spouse, $dgenerations, $bwidth, $bheight, $TEXT_DIRECTION, $GM_IMAGE_DIR, $GM_IMAGES, $generations, $box_width, $view, $show_full, $gm_lang;
			...
			for($i=0; $i<$ct; $i++) {
				...
				if ($ct>1) {
					if ($i==0) {
						...
						print "<tr><td height=\"50%\" style=\"background: url('".$GM_IMAGE_DIR."/".$GM_IMAGES["vline"]["other"]."');\"><img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["spacer"]["other"]."\" width=\"3\" alt=\"\" /></td>\n";
					}
					else if ($i==$ct-1) {
						...
						print "<tr><td height=\"50%\"><img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["spacer"]["other"]."\" width=\"3\" alt=\"\" /></td>\n";
					}
					else {
						print "<td style=\"background: url('".$GM_IMAGE_DIR."/".$GM_IMAGES["vline"]["other"]."');\"><img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["spacer"]["other"]."\" width=\"3\" alt=\"\" /></td>\n";
					}
				}
				print "</tr>\n";
			}
	...
	if ($count==0) {
		...
		if (displayDetailsByID($pid) || showLivingNameByID($pid)) {
			...
			if ($famids||($num>1)) {
				...
				if ($view!="preview") {
					print "<a href=\"javascript: ".$gm_lang["show"]."\" onclick=\"return togglechildrenbox();\" onmouseover=\"swap_image('larrow',3);\" onmouseout=\"swap_image('larrow',3);\">";
					print "<img id=\"larrow\" src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["darrow"]["other"]."\" border=\"0\" alt=\"\" />";
					print "</a>";
				}
				print "\n\t\t<div id=\"childbox\" dir=\"";
...
function print_person_pedigree($pid, $count) {
	global $generations, $SHOW_EMPTY_BOXES, $GM_IMAGE_DIR, $GM_IMAGES, $bheight;
	...
	foreach($famids as $indexval => $famid) {
		...
		$height="100%";
		print "<tr>";
		if ($count<$generations-1) print "<td height=\"50%\"><img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["spacer"]["other"]."\" width=\"3\" alt=\"\" /></td>\n";
		if ($count<$generations-1) print "<td rowspan=\"2\"><img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["hline"]["other"]."\" width=\"7\" height=\"3\" alt=\"\" /></td>\n";
		print "<td rowspan=\"2\">\n";
		...
		print "</td>\n";
		print "</tr>\n<tr>\n<td height=\"50%\"";
		if ($count<$generations-1) print " style=\"background: url('".$GM_IMAGE_DIR."/".$GM_IMAGES["vline"]["other"]."');\" ";
		print "><img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["spacer"]["other"]."\" width=\"3\" alt=\"\" /></td></tr>\n<tr>\n";
		if ($count<$generations-1) print "<td height=\"50%\" style=\"background: url('".$GM_IMAGE_DIR."/".$GM_IMAGES["vline"]["other"]."');\"><img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["spacer"]["other"]."\" width=\"3\" alt=\"\" /></td>";
		if ($count<$generations-1) print "<td rowspan=\"2\"><img src=\"".$GM_IMAGE_DIR."/".$GM_IMAGES["hline"]["other"]."\" width=\"7\" height=\"3\" alt=\"\" /></td>\n";
		print "<td rowspan=\"2\">\n";
...
{
    global $generations, $dgenerations, $gm_lang, $firstrun;
	if ($descent==0) return;
    $famids = find_sfamily_ids($pid);
...
print_header(PrintReady($name)." ".$gm_lang["familybook_chart"]);
print "\n\t<table width=\"100%\" class=\"list_table $TEXT_DIRECTION\"><tr><td valign=\"top\">\n\t\t";
if ($view!="preview") print "\n\t<h2>".$gm_lang["familybook_chart"].":<br />".PrintReady($name)."</h2>";
else print "\n\t<h2 style=\"text-align: center\">".$gm_lang["familybook_chart"].":<br />".PrintReady($name)."</h2>";
?>
...
if ($view!="preview") {
	...
	print "<td class=\"list_label\">&nbsp;" . $gm_lang["root_person"] . "&nbsp;</td>";
	print "<td class=\"list_value\">";
	print "\n\t\t<input class=\"pedigree_form\" type=\"text\" name=\"pid\" size=\"3\" value=\"$pid\" />";
	print "<font size=\"1\"> <a href=\"javascript:open_find(document.people.pid);\">".$gm_lang["find_id"]."</a></font>";
	...
	print "</td>";
	print "</tr><tr>";
	print "<td class=\"list_label\" >&nbsp;" . $gm_lang["generations"] . "&nbsp;</td>";
	print "<td class=\"list_value\">";
//	print "<input type=\"text\" size=\"3\" name=\"generations\" value=\"$generations\" />";
	...
	print "</td>";
	print "</tr><tr>";
	print "<td class=\"list_label\">&nbsp;" . $gm_lang["box_width"] . "&nbsp;</td>";
