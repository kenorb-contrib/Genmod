 ...
 * @package Genmod
 * @subpackage Edit
 * @version $Id: edit_interface.php,v 1.154 2009/04/03 12:07:21 sjouke Exp $
 */
...
//				$fact = "NOTE";
	...
	case "addnewgnote":
		?>
		<b><?php print $gm_lang["create_gnote"];
		$tabkey = 1;
		 ?></b>
...
//		AddTagSeparator("create_gnote");
		...
		if ($Users->UserCanAccept($gm_username) && !$Users->userAutoAccept($gm_username)) print "<br /><input name=\"aa_attempt\" type=\"checkbox\" value=\"1\" />".$gm_lang["attempt_auto_acc"]."<br />";
		print "<br /><input class=\"center\" type=\"submit\" value=\"".$gm_lang["create_gnote"]."\" /><br />";
		print "</form>";
		print "\n<script type=\"text/javascript\">\n<!--\ndocument.getElementById(\"".$element_id."\").focus();\n//-->\n</script>";
		break;
...
//						ShowMediaForm($pid);
			...
			if ($orgfact != "SEX" && $orgfact != "RESN" && $rectype != "OBJE" && $rectype != "NOTE") {
				...
				if ($orgfact!="OBJE"  && $orgfact!="REPO" && $orgfact!="NOTE" && $MULTI_MEDIA) PrintAddLayer("OBJE");
				if ($orgfact!="NOTE") {
					PrintAddLayer("NOTE");
					PrintAddLayer("GNOTE");
				}
...
if (isset($change_id) && $can_auto_accept &&  (($Users->UserCanAccept($gm_username) && $aa_attempt) || $Users->userAutoAccept())) {
	AcceptChange($change_id, $GEDCOMID);
}

// autoclose window when update successful
if ($success and $EDIT_AUTOCLOSE) {
	session_write_close();
	print "\n<script type=\"text/javascript\">\n<!--\nedit_close();\n//-->\n</script>";
}
